...
- **Observability** (`config/MetricsConfiguration.java`): JHipster's Prometheus metrics (`/management/prometheus`, JVM/HTTP/process, percentile histograms) plus **app-level instrumentation** — a `ankobra.enquiries.submitted` counter (tagged by need) + `@Timed` on `PublicEnquiryResource`, and `@Observed` on `PortalSecurityService`'s authz methods. `@Timed`/`@Counted`/`@Observed` are enabled by the aspect beans in `MetricsConfiguration`. **Distributed tracing** is Micrometer Tracing → OpenTelemetry over OTLP (`micrometer-tracing-bridge-otel` + `opentelemetry-exporter-otlp`), config-gated via `OTEL_*` env (on in dev at 100% sampling; opt-in in prod — the single-container deploy has no collector). Trace/span ids are woven into the console log pattern (`logback-spring.xml`). Tracing is disabled in the test profile. **These files (`pom.xml` deps, the three `application*.yml` `management.tracing`/`otlp` blocks, `logback-spring.xml`, `MetricsConfiguration`) are custom — re-apply after regeneration.**
