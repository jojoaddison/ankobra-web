...
- **Verified green**: `./mvnw verify` (547 tests) and `npm test` (130 files, lint clean). Angular runtime is pinned at 21.2.19+ / tooling 21.2.20+ to clear the XSS sanitization-bypass advisories; `npm audit --omit=dev` is clean and should stay that way.
