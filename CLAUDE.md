...
- **Hardening** (audit P1, `docs/security-20260805-0936.md`): `security/LoginAttemptService` throttles failed logins per source IP **and** per username with a progressive lockout, checked in `AuthenticateController` before authentication (429, not 401). It is **in-memory and per-instance** — scaling out means moving it to Redis or the DB. `security/PasswordPolicy` enforces a 12-char floor plus a denylist and login-containment check. The public enquiry endpoint carries an off-screen honeypot field (`website`) whose submissions are discarded with a 201 and no counter increment. nginx adds HSTS (with `includeSubDomains` since 2026-08-24, after auditing all seven `*.jojoaddison.net` hosts; still no `preload`), `server_tokens off`, a body cap and `limit_req` zones — the zones live in `deploy/prod-server/ankobra-web-limits.conf`, installed to `conf.d/` because `limit_req_zone` is only valid in `http{}`; `deploy.sh --with-nginx` ships it **before** the site file or `nginx -t` fails on the missing zone. The password policy also runs a **HIBP k-anonymity breach check** (`BreachedPasswordChecker`, fail-open with a counter and an audit line, disabled in the test profile), and `jhi_user.must_change_password` + `PasswordChangeRequiredFilter` hold accounts whose password predates the 12-char floor at the password screen.
- **Session revocation** (SEC-09): `jhi_user.token_version` is stamped into every JWT as a `tv` claim; `security/TokenVersionValidator` (delegating alongside `JwtValidators.createDefault()` in `SecurityJwtConfiguration`) rejects tokens issued before a user's current generation. Bumping one user signs out only that user. Fires on password change/reset, on the active→inactive transition in an admin edit, and via `POST /api/admin/users/{login}/revoke-sessions`. **Always revoke through `UserService.revokeSessions`** — `findOneByLogin` is `@Cacheable` and the validator reads through it, so a raw SQL bump is silently ineffective for the cache TTL. A missing `tv` claim is read as 0 so a deploy does not sign everyone out.
- **Session transport** (SEC-06): the JWT lives in an **`HttpOnly; SameSite=Strict` cookie** (`AccessTokenCookie`), not web storage — nothing on the client can read it, and logout is a server call (`POST /api/logout`) because script cannot delete such a cookie. CSRF protection is therefore **on** (double-submit `XSRF-TOKEN`/`X-XSRF-TOKEN`, Angular's defaults, so no client code); the token must be issued eagerly (`setCsrfRequestAttributeName(null)`) or the cookie is never written for an SPA. **The trap:** reading the cookie in a `BearerTokenResolver` silently disables CSRF for every authenticated request, because `OAuth2ResourceServerConfigurer` exempts any request its resolver can read a token from — correct for headers, a total bypass for cookies. Hence `CookieAccessTokenFilter`, registered **after `CsrfFilter`**; that ordering is the mechanism, not a detail. The whole test suite stayed green while the bypass was live, so `CsrfProtectionIT` logs in for real and replays a request with the session and without the token.
- **CSP and the Angular build** (SEC-06): `script-src` is `'self'` with **no `'unsafe-inline'`**, which constrains the build. Angular's `inlineCritical` optimisation emits `<link rel="stylesheet" media="print" onload="this.media='all'">` — an inline event handler that a strict `script-src` blocks, leaving the stylesheet stuck at `media="print"` so every Bootstrap-styled page (login, admin, `/cms`) renders unstyled while the bespoke marketing front still looks fine. It is therefore **disabled** in `angular.json` (`production.optimization.styles.inlineCritical: false`; minification and the script/font optimisations stay on). The one inline script the page had lives in `content/loading-error.js` for the same reason. Re-enabling either silently requires `'unsafe-inline'` back, which makes CSP decoration rather than a control. Only a real browser catches this — the built HTML and every test look fine. `style-src` is nonce-based, and since 2026-08-24 there is **no `style-src-attr` and no `'unsafe-inline'` anywhere in the policy**: all 43 inline `style="…"` attributes became classes (`.pgrid-*`, `.pcard-*`, `.pt-*` in `global.scss`; `quotes.scss`; `home.scss`; the shell's nine in `content/css/loading.css`). A stray `style="…"` is now _silently dropped by the browser_ — the element renders unstyled and nothing errors — so `ContentSecurityPolicyConfigTest` sweeps every template for one.
- **Verified green**: `./mvnw verify` (586 tests) and `npm test` (131 files, lint clean). Note the test database is **file-backed** (`target/h2db/testdb`) and survives between runs — a row committed by a failing test will fail unrelated tests next run; `rm -rf target/h2db/testdb` clears it. Angular runtime is pinned at 21.2.19+ / tooling 21.2.20+ to clear the XSS sanitization-bypass advisories; `npm audit --omit=dev` is clean and CI now fails if it stops being.

When regenerating entities via JDL, re-apply: the five scoped resources (Project/Ticket/Client/Quote + `PublicEnquiryResource`), the `@WithMockUser(authorities = "ROLE_CONSULTANT")` on **ten** generated `ResourceIT`s (Project/Ticket/Client/Quote/Lead/Course/Milestone/QuoteLine/ServiceItem/TeamMember — the generator emits a bare `@WithMockUser`, i.e. `ROLE_USER`, which the authorization rules below now correctly reject), the **domain API authorization block in `SecurityConfiguration`** and the per-object checks in `TicketResource` (see below), `User.toString()` with `activationKey` removed (the generator emits it, and UserService logs whole `User` objects in six places — see `docs/security-20260805-0936.md`), the rest of `SecurityConfiguration`'s customisations — **CSRF enabled with eager token issuance**, `CookieAccessTokenFilter` registered _after_ `CsrfFilter`, `PasswordChangeRequiredFilter` last, eager header writing, the nonce CSP writer and the disabled HSTS writer — plus `AuthenticateController`'s cookie issuance and `POST /api/logout`, the full-bleed `main.html`/hidden-navbar tweaks, and the **CMS nesting**: move the 10 domain entities out of `entities/entity.routes.ts` back into `cms/cms.routes.ts` children, and re-prefix the regenerated `['/<entity>', …]` links in each entity list/detail template to `['/cms/<entity>', …]`. The originating reference files remain the spec:
